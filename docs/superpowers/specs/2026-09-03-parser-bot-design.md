...
| `lf_api.py` | LF client. `POST /api/oauth2/v1/{ns}/token` (password grant with the admin's *personal* login email; `ns` = `Company.LOAN_FACTORY` = `5716104026521600`), then `POST /api/mobile/v1/{ns}/getCurrentUserAPIKey` → `x-api-key`, then `POST /api/lfiq/v1/{ns}/execute/FindOp` `{kind: RateUpdate, created: [day, day], l: 500, o: [-created]}`; `status == false` and the exact `since` instant are filtered client-side (FindOp cannot combine `status` with a `created` range; the un-namespaced EntityAPI cannot verify LF-namespace tokens). `created` on the wire is epoch millis. |
